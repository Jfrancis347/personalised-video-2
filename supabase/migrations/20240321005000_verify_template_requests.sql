-- Verify and fix template_requests table
DO $$
BEGIN
    ...
    DROP POLICY IF EXISTS "Users can view own template requests" ON public.template_requests;
    DROP POLICY IF EXISTS "Users can create template requests" ON public.template_requests;
    DROP POLICY IF EXISTS "Admin full access to template requests" ON public.template_requests;
    ...
    -- Create indexes
    CREATE INDEX IF NOT EXISTS idx_template_requests_user_id 
        ON public.template_requests(user_id);
    CREATE INDEX IF NOT EXISTS idx_template_requests_status 
        ON public.template_requests(status);
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'No policies found for template_requests';
    END IF;
END $$;
